...
.. math::
    \hat{V}^{\pi}_{DR}
    &=\frac{1}{T}\sum_{t=1}^T\sum_{a=0}^m \left\{ (Y_t-\hat{\mu}(a|X_t)) D_{ta} \frac{\pi(a|X_t)}{\pi_b(a|X_t)} + \pi(a|X_t)\hat{\mu}(a|X_t) \right\}\\
    &=\hat{V}^{\pi}_{DM} + \frac{1}{T}\sum_{t=1}^T\sum_{a=0}^m  (Y_t-\hat{\mu}(a|X_t)) D_{ta} \frac{\pi(a|X_t)}{\pi_b(a|X_t)}  .
